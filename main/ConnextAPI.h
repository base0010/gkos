...
class ConnextAPI
{
  ...
  public:
    ...
    string AuthResponse(string nonce, string address, string origin, string signature);
    //get the challenge Hash
    void AuthorizeWithHub();
    string GetAuthHex();
    uint8_t* GetAuthHashHex(string challenge);
    string SignHash(uint8_t*);
    void StoreAuthToken(string authResponse);
    void GetChannel();
    string SendRequestDeposit(signedDepositReq_t depositReq, int txCount, int lastThreadUpdateId);
    //Remove
    ...
    string GenerateSateHash(state_t);

    string Update(int updateNum, hubUpdate_t *update);


    string SignState(string stateHash);
    ...
    string GenerateLongSateHash(longState_t *state, string *userAddress, string *recipientAddress);
    ...
    string httpGET(const char *endpoint, const char *cookie, bool isJson = true);





    string GetChannelJson(string address);

    string httpPOST(const char *endpoint, const char *postData, bool isJson = true, const char *cookie = "");
    ...
    string ECRecoverState(longState_t *state, string sig);

    string UserAuthorizedUpdate(longState_t *state, string sigHub);
    ...
    string RequestCollateral(string user, int lastChannelTx);

    string ApproveTokens(uint32_t aTokens);
